...
class Global(tf.keras.layers.Layer):
    def __init__(self, max_subs=Max_S, latent_dim=64, name='Global', **kwargs):
        ...
        self.dense_1 = tf.keras.layers.Dense(64, activation='relu',
                                             name='globaldense1')  # tf.keras.layers.BatchNormalization(),
...
class Seq(tf.keras.Model):
    def __init__(self, latent_dim=64, intermediate_dim=128, name='Seq', **kwargs):
        ...
        self.out = tf.keras.layers.Dense(2, activation='sigmoid', name='posout')
        self.mode_dense1 = tf.keras.layers.Dense(32, activation='relu', name='mode_fc1')
        self.mode_dense2 = tf.keras.layers.Dense(16, activation='relu', name='mode_fc2')
        self.mode = tf.keras.layers.Dense(mode_class_num, activation='softmax', name='modeout')
    ...
    def call(self, inputs):
        ...
        a, x = self.pred(x)
        x = self.out(a * x)
        _ = self.mode_dense1(a * x)
        _ = self.mode_dense2(_)
        m = self.mode(_)
